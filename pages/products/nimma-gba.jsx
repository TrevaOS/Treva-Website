    ...
    <>
        ...
        <div className="mx-auto grid max-w-7xl grid-cols-1 items-center gap-14 px-6 lg:grid-cols-2">
          <div>
            ...
            <motion.h1
              ...
              className="mb-4 font-black text-gray-900"
            ...
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}
              className="mb-3 text-xl font-500 italic text-gray-500">
              One platform for Karnataka government civic issues.
            </motion.p>
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.25 }}
              className="mb-8 max-w-lg text-base leading-relaxed text-gray-500">
          ...
          >
            <div className="mb-5 flex items-center justify-between">
              ...
              <span className="rounded-full bg-gray-100 px-3 py-1 text-xs text-gray-500">Step 3 of 4</span>
            </div>
            <div className="space-y-4">
              {issueTypes.map(({ icon: Icon, label, detail }) => (
                <div key={label} className="flex items-center gap-4 rounded-xl bg-gray-100 p-4">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="text-sm font-700 text-gray-900">{label}</div>
              ...
              <div className="rounded-xl bg-gray-100 p-4">
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div className="aspect-square rounded-lg border border-dashed border-white/15 bg-black/40" />
                  <div className="flex aspect-square items-center justify-center rounded-lg border border-dashed bg-black/40 text-xs text-gray-500" style={{ borderColor: `${green}35` }}>Add</div>
                </div>
              </div>
            </div>
          </motion.div>
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            {features.map(({ icon: Icon, title, desc }, i) => (
              <FadeIn key={title} delay={i * 0.08}>
                <div className="card-glow h-full rounded-2xl border bg-white p-7" style={{ borderColor: `${green}12` }}>
                  ...
                  <h3 className="mb-2 text-lg font-800 text-gray-900">{title}</h3>
      ...
      <section id="waitlist" className="bg-white py-20">
        <div className="mx-auto max-w-3xl px-6">
          <FadeIn>
            <div className="card-glow rounded-2xl border border-gray-100 bg-gray-50 p-8 md:p-10">
              <h2 className="mb-3 text-3xl font-black text-gray-900">Get Nimma GBA Updates</h2>
              <p className="mb-8 text-sm leading-relaxed text-gray-500">
                Join the early interest list for launch updates, partnership discussions, and product demos.
              </p>

              {isSubmitSuccessful ? (
                <div className="rounded-xl border p-5 text-sm font-600 text-gray-900" style={{ borderColor: `${green}35`, background: `${green}12` }}>
                  Thanks. We have received your interest in Nimma GBA.
                </div>
              ) : (
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <input {...register('name', { required: true })} placeholder="Name" className="rounded-xl border border-white/10 bg-black px-4 py-3 text-sm text-gray-900 outline-none focus:border-[#29C8D5]" />
                    <input {...register('email', { required: true })} type="email" placeholder="Email" className="rounded-xl border border-white/10 bg-black px-4 py-3 text-sm text-gray-900 outline-none focus:border-[#29C8D5]" />
                  </div>
                  <input {...register('organization')} placeholder="Organization or role" className="w-full rounded-xl border border-white/10 bg-black px-4 py-3 text-sm text-gray-900 outline-none focus:border-[#29C8D5]" />
                  <textarea {...register('message')} rows={4} placeholder="Tell us what you want to explore" className="w-full rounded-xl border border-white/10 bg-black px-4 py-3 text-sm text-gray-900 outline-none focus:border-[#29C8D5]" />
